...
CREATE TABLE users (
    ...
    last_connection_date DATETIME DEFAULT CURRENT_TIMESTAMP
);
...
INSERT INTO user (email_address, password, file_as, user_status, password_expiry_date, failed_login) VALUES ('admin@encorefinancials.com', '68aff5412f35ed76', 'Administrator', 'Active', '2022-12-30', 0);
...
CREATE PROCEDURE update_user_password(IN p_user_id INT(10), IN p_email_address VARCHAR(100), p_password VARCHAR(200), p_password_expiry_date DATE)
BEGIN
	UPDATE users 
...
CREATE PROCEDURE update_user(IN p_user_id INT(10), IN p_email_address VARCHAR(100), IN p_password VARCHAR(200), IN p_file_as VARCHAR (300), IN p_password_expiry_date DATE)
BEGIN
...
CREATE PROCEDURE insert_user(IN p_email_address VARCHAR(100), IN p_password VARCHAR(200), IN p_file_as VARCHAR (300), IN p_password_expiry_date DATE)
BEGIN
